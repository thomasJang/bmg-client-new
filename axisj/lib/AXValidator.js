...
var AXValidator = Class.create(AXJ, {
	...
	init: function () {
		...
		this.form = axdom(document[cfg.targetFormName]);
		this.findElement();
	},
			...
			try {
				...
				jQuery.each(checkClass, function (k, v) {
					if (Elem.id) {
						...
						if (axdom(document[cfg.targetFormName][Elem.name]).hasClass(cfg.clazz + k)) {
							config[k] = v;
							isValidate = true;
						}
					}
	...
	del: function (delObj) {
		...
		var findIndex = null;
		if (delObj.id) {
			axf.each(this.elements, function (eidx, elem) {
		...
		if (findIndex != null) {
			...
			axdom("#" + deledObject.id).unbind("keydown.validate");
			axdom("#" + deledObject.id).unbind("keyup.validate");
			axdom("#" + deledObject.id).unbind("blur.validate");
	...
	add: function (addObj) {
		...
		if (addObj.id) {
			...
			if (findIndex != null) {
				...
				addObj.id = axdom(document[cfg.targetFormName][addObj.name]).attr("id");
				this.elements.push(addObj);
				addedObject = this.elements.last();
			}
		...
		if (addedObject.id) {
			...
			targetElem = axdom(document[cfg.targetFormName][addedObject.name]);
	...
	getErrorElement: function () {
		...
		var errObj = this.errElements.last();
		var errElement = errObj.element;
		return jQuery("#" + errElement.id).length > 0 ? jQuery("#" + errElement.id) : axdom(document[cfg.targetFormName][errElement.name]);
	},
